<script>
  ...
  $: plugin = $plugins.find(p => p.name === 'block5xx');
...
<div class="plugin-page">
  <div class="plugin-header">
    <div class="plugin-title-row">
      <h1>Block 5xx Plugin</h1>
      <span class="plugin-status" class:enabled={plugin?.enabled}>
        {plugin?.enabled ? 'Enabled' : 'Disabled'}
      </span>
    </div>
    <p class="plugin-description-text">
      {plugin?.description || 'Intercepts 5xx errors and returns custom responses'}
    </p>
  </div>
  ...
  <div class="plugin-section">
    ...
    <Card>
      <div class="about-content">
        ...
        <p><strong>Purpose:</strong> Prevents 5xx server errors from reaching the client by intercepting and replacing them.</p>
        <p><strong>Impact:</strong> Modifies response status and body when 5xx errors are detected.</p>
        <p><strong>Use Case:</strong> Useful for testing error handling in frontend applications without backend failures.</p>
      </div>
    </Card>
  </div>
</div>
